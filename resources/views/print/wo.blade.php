...
<div class="print-header">
    ...
    <div>
        ...
        <div class="doc-number">{{ $workOrder->wo_number }}</div>
    </div>
</div>
<table class="info-table">
    <tr>
        <td class="label">Unit</td>
        <td><strong>{{ $workOrder->unit->unit_code }}</strong> - {{ $workOrder->unit->unit_model }}</td>
        <td class="label">Type</td>
        <td>{{ ucfirst($workOrder->maintenance_type) }}</td>
    </tr>
    <tr>
        <td class="label">Technician</td>
        <td>{{ $workOrder->technician->technician_name ?? '-' }}</td>
        <td class="label">Status</td>
        <td>{{ ucwords(str_replace('_',' ',$workOrder->status)) }}</td>
    </tr>
    <tr>
...
<table class="info-table" style="width:300px;">
    ...
    <tr>
        ...
        <td style="text-align:right;">IDR {{ number_format($cs->sparepart_cost ?? 0, 0, ',', '.') }}</td>
    </tr>
    <tr>
        <td class="label">Labor</td>
        <td style="text-align:right;">IDR {{ number_format($workOrder->labor_cost, 0, ',', '.') }}</td>
    </tr>
    <tr>
        <td class="label">Vendor</td>
        <td style="text-align:right;">IDR {{ number_format($workOrder->vendor_cost, 0, ',', '.') }}</td>
    </tr>
    <tr>
        <td class="label">Consumable</td>
        <td style="text-align:right;">IDR {{ number_format($workOrder->consumable_cost, 0, ',', '.') }}</td>
    </tr>
    <tr style="border-top:2px solid #111;">
        <td class="label"><strong>TOTAL</strong></td>
        <td style="text-align:right;"><strong>IDR {{ number_format(($cs->total_cost ?? 0), 0, ',', '.') }}</strong></td>
    </tr>
</table>
<div class="sign-area">
    <div class="sign-box">
        <div>Technician</div>
        <div class="sign-line">{{ $workOrder->technician->technician_name ?? '______' }}</div>
    </div>
    <div class="sign-box">
        <div>Supervisor</div>
        <div class="sign-line">______</div>
    </div>
</div>
@endsection
