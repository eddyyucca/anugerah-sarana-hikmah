        ...
        <div class="erp-card p-3">
            ...
            <table class="table table-sm mb-3">
                <tr>
                    ...
                    <td>{{ $stockOpname->warehouseLocation->name ?? 'All' }}</td>
                </tr>
                <tr>
                    <td class="text-muted">Conducted</td>
                    <td>{{ $stockOpname->conductor->name ?? '-' }}</td>
                </tr>
                @if($stockOpname->approver)
                <tr>
                    <td class="text-muted">Approved</td>
                    <td>{{ $stockOpname->approver->name }}</td>
                </tr>
                @endif
            </table>
            @if($stockOpname->status === 'draft')
            @php
                $canApprove = auth()->user()->canApproveDocument('so', $stockOpname->id);
            @endphp
            @if($canApprove)
                <form action="{{ route('stock-opname.approve', $stockOpname) }}" method="POST">@csrf
                    <button class="btn btn-sm btn-success" onclick="return confirm('Approve? Stock akan disesuaikan.')"><i class="bi bi-check-lg me-1"></i>Approve & Adjust Stock</button>
                </form>
            @else
                <button class="btn btn-sm btn-success" disabled title="You don't have approval permission"><i class="bi bi-check-lg me-1"></i>Approve & Adjust Stock</button>
            @endif
            @endif
        </div>
    </div>
