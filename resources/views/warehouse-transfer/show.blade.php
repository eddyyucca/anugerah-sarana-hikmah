    ...
    <div class="col-lg-4">
        <div class="erp-card p-3 mb-3">
            <div class="d-flex justify-content-between mb-3">
                <div>
                    <div style="font-weight:800;font-size:1.1rem;">{{ $warehouseTransfer->transfer_number }}</div>
                </div>
            ...
            <table class="table table-sm mb-3">
                <tr>
                    ...
                    <td><strong>{{ $warehouseTransfer->fromLocation->name ?? '-' }}</strong></td>
                </tr>
                <tr>
                    <td class="text-muted">To</td>
                    <td><strong>{{ $warehouseTransfer->toLocation->name ?? '-' }}</strong></td>
                </tr>
                <tr>
                    <td class="text-muted">Date</td>
                    <td>{{ $warehouseTransfer->transfer_date->format('d M Y') }}</td>
                </tr>
                <tr>
                    <td class="text-muted">Remarks</td>
                    <td>{{ $warehouseTransfer->remarks ?? '-' }}</td>
                </tr>
                <tr>
                    <td class="text-muted">Created By</td>
                    <td>{{ $warehouseTransfer->creator->name ?? '-' }}</td>
                </tr>
                @if($warehouseTransfer->poster)
                <tr>
                    <td class="text-muted">Sent By</td>
                    <td>
                        {{ $warehouseTransfer->poster->name }}
                        <div class="text-muted" style="font-size:.75rem;">{{ $warehouseTransfer->posted_at?->format('d M Y H:i') }}</div>
                    </td>
                </tr>
                @endif
                ...
                <tr>
                    ...
                    <td>
                        {{ $warehouseTransfer->receiver->name }}
                        <div class="text-muted" style="font-size:.75rem;">{{ $warehouseTransfer->received_at?->format('d M Y H:i') }}</div>
                    </td>
                </tr>
                @endif
            </table>
            ...
            <div class="d-flex flex-wrap gap-2">
                ...
                <form action="{{ route('warehouse-transfer.send', $warehouseTransfer) }}" method="POST">@csrf
                    <button class="btn btn-sm btn-warning" onclick="return confirm('Send transfer? Stock will be deducted from source.')"><i class="bi bi-send me-1"></i>Send</button>
                </form>
                @endif
                @if($warehouseTransfer->status === 'sent')
                <form action="{{ route('warehouse-transfer.receive', $warehouseTransfer) }}" method="POST">@csrf
                    <button class="btn btn-sm btn-success" onclick="return confirm('Confirm receipt? Stock will be added to destination.')"><i class="bi bi-check-lg me-1"></i>Receive</button>
                </form>
                @endif
            </div>
        </div>
