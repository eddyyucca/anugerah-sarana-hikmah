...
<div class="row g-3">
        ...
        <x-card class="p-3 mb-3">
            ...
            <div class="d-flex flex-wrap gap-2">
                @if($workOrder->status === 'open')
                    <form action="{{ route('work-orders.progress', $workOrder) }}" method="POST">@csrf<x-button type="submit" variant="warning" size="sm">Start Progress</x-button></form>
                @endif
                @if(in_array($workOrder->status, ['in_progress','waiting_part']))
                    <form action="{{ route('work-orders.complete', $workOrder) }}" method="POST">@csrf<x-button type="submit" variant="success" size="sm" onclick="return confirm('Complete this WO?')"><i class="bi bi-check-lg me-1"></i>Complete</x-button></form>
                @endif
                @if(!in_array($workOrder->status, ['completed','cancelled']))
                    ...
                    <a href="{{ route('goods-issues.create', ['wo_id' => $workOrder->id]) }}" class="btn btn-sm btn-outline-danger"><i class="bi bi-box-arrow-up me-1"></i>Issue Parts</a>
                @endif
            </div>
        </x-card>
    ...
    <div class="col-lg-8">
        <x-card class="mb-3">
            <x-slot:header>
                <div class="section-title">Related Goods Issues</div>
            </x-slot:header>
